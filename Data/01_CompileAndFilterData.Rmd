...
ls_tx_ga<-read.csv("limnosat_large/ls_tx_ga.csv", row.names = 1) %>% select(-c(lagoslakeid, LandsatID)) %>% mutate(date=as.Date(date, format="%Y-%m-%d"))
...
write.csv(full_limnosat,"2024-07-30_full_limnosat.csv")
```

# 4. Filter and create spatial and temporal dataframes 
...
new_limnosat<-read.csv("2024-07-30_full_limnosat.csv",row.names=1)
lake_data<-readRDS("2023-09-29_lagos_landcover_data.rds")


#Filter Limnosat data by observations between May 1st October 31st
new_limnosat$date_m_d <- format(as.POSIXct(new_limnosat$date), format = "%m-%d")
