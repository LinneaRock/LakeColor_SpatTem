...
saveRDS(elev_data, 'LakeSR/1_DataPrep/1_DataDownloads/1_LAGOS/elev_data.RDS')
...
saveRDS(hydro_data, 'LakeSR/1_DataPrep/1_DataDownloads/1_LAGOS/hydro_data.RDS')
...
saveRDS(watershed_data, 'LakeSR/1_DataPrep/1_DataDownloads/1_LAGOS/watershed_data.RDS')


link <- dt4
rm(dt4)

# Linnea and Jordy added in pulling Texas lakes here, but Jordy removed July 16 2026 after we got full lake SR datasets. Still downloaded lake_link incase we need it.

link <- link |>
        select(lagoslakeid, lake_nhdid, lake_centroidstate, nhdplusv2_comid)
saveRDS(link, 'LakeSR/1_DataPrep/1_DataDownloads/1_LAGOS/lake_link.RDS')
