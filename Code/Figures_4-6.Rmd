...
```{r}
readRDS("2024-08-02_lakecolor_climate_landcover_trends.rds
```
...
lk <- lk %>% filter(avg_dwl_site < 590) # remove lakes with wavelengths greater than 590
nrow(lk) # 81867
...
set.seed(111)
(rF_color <- randomForest(color_category ~ ., data = lake_color, mtry = 5))
...
# Blue  28918  6097   0.1741254
# Green  5795 40981   0.1238883
# 
...
# No. of variables tried at each split: 5
# 
#         OOB estimate of  error rate: 53.06%
# Confusion matrix:
#                No trend Trending blue Trending green class.error
# No trend            931           800            717   0.6196895
# Trending blue       667          1346            441   0.4515077
# Trending green      722           550           1170   0.5208845

rm(lake_color);rm(validate);rm(train);rm(lk);rm(data_subset);rm(category_counts)
...
rF_imp <- rF_imp %>% arrange(MeanDecreaseGini) %>%  mutate(newvarnames = factor(newvarnames, levels = newvarnames)) %>% slice(10)
...
ggsave("/Users/jordanscheibe/Library/CloudStorage/OneDrive-UniversityofWyoming/Collaborations/LakeColor/LakeColor_SpatTem/Figures/Figure4/Fig4_Importance_rF_trend.png",width=6.7, height=8, units="in",dpi = 300)
...
ggsave("/Users/jordanscheibe/Library/CloudStorage/OneDrive-UniversityofWyoming/Collaborations/LakeColor/LakeColor_SpatTem/Figures/Figure4/Fig4_Importance_rF_color.png",width=6.7, height=8, units="in",dpi = 300)
```
